...
import com.example.masterplanbbe.infrastructure.exception.chat.ChatMessageQueryException;
...
public class ChatMessageRepositoryImpl implements ChatMessageRepositoryCustom {
    ...
    public Slice<ChatResponse> findChatList(Long lastChatId, Long specId, Pageable pageable) {
        try {
            ...
            log.error("채팅 메시지 쿼리 실행 중 오류 발생: {}", e.getMessage(), e);
            throw new ChatMessageQueryException(); // 쿼리 실행 실패 시 예외 처리
        }
    }
}
